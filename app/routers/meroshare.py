...
def apply_for_share(db: Session = Depends(get_db), meroshare_id: int = None, request: Request = None, current_user: User = Depends(get_current_user)):
    ...
    jwt_token = ""
    auth_response = safe_http_request(method='post', url=f'{url}/api/meroShare/auth/', json={"clientId": meroshare.client_id,"username": username,"password": password}, auth=True)
    
    if auth_response["success"]:
        jwt_token = auth_response['auth']
    else:
        print(auth_response["message"])
        return success_response(message='Failed to authenticate with Meroshare API')
    ...
    if len(ipolist) == 0:
        return success_response(message='No applicable shares found for this Meroshare account')
                ...
                if check_response['data'].get("status", {}) == 'ACCEPTED':
                    ...
                    if float(ipo_details.get('sharePerUnit')) < 200 or float(ipo_details.get('sharePerUnit')) > 90:
                        ...
                        if response_ipo['success']:
                            return success_response(message='Successfully applied for the share')
                        else:
                            return success_response(message='Failed to apply for the share')
        else:
            return success_response(message='Already applied for this share')
    return success_response(message='Completed applying for shares')
